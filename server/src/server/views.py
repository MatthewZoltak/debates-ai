    ...
    async with async_session() as session:
        ...
        if not api_key_record or api_key_record.user_id != user_id:
            return web.json_response(
                {"error": "API key not found or access denied."}, status=403
            )
        try:
            api_key = decrypt_api_key(api_key_record.api_key_encrypted)
        except Exception:
            return web.json_response(
                {"error": "Invalid or corrupted API key."}, status=403
            )
...
async def get_user_api_key_view(request):
    user_id = request["user_id"]
    async with async_session() as session:
        existing = await get_items_by_filters(
            session, db_models.UserAPIKey, user_id=user_id, provider="gemini"
        )
        if not existing:
        ...
        for key in existing:
            try:
                ...
                valid_keys.append(
                    {"id": key.id, "nickname": key.nickname, "provider": key.provider}
                )
            except Exception:
                continue
...
@docs(
    ...
    responses={
        200: {"schema": UserAPIKeyListResponse, "description": "List of API keys"}
    },
)
async def list_user_api_keys_view(request):
    user_id = request["user_id"]
    async with async_session() as session:
        keys = await get_items_by_filters(
            session, db_models.UserAPIKey, user_id=user_id
        )
        result = [
            {
                "id": k.id,
...
@docs(
    ...
    responses={
        200: {
            "schema": UserAPIKeyValueResponse,
            "description": "Decrypted API key value",
        },
        404: {"description": "Not found"},
    },
)
async def get_user_api_key_value_view(request):
    ...
    response_data = UserAPIKeyValueResponse().dump(
        {
            "id": key.id,
...
async def get_public_debates(request) -> web.Response:
    ...
    async with async_session() as session:
        ...
        count_query = select(func.count(db_models.Debate.id)).where(
            db_models.Debate.is_public.is_(True)
        )
        total_count = await session.scalar(count_query)

        # Build query with sorting
        query = select(db_models.Debate).where(db_models.Debate.is_public.is_(True))

        if sort_by == "likes":
            # Sort by like count (subquery to count likes)
            like_count = (
                select(func.count(db_models.UserLike.id))
                .where(db_models.UserLike.debate_id == db_models.Debate.id)
                .scalar_subquery()
            )
...
async def toggle_debate_visibility_view(request) -> web.Response:
    ...
    async with async_session() as session:
        ...
        if is_public and not debate.winner:
            return web.json_response(
                {"error": "Only completed debates can be made public"}, status=400
            )

        await update_item(
            session, debate_id, {"is_public": is_public}, db_models.Debate
        )
...
async def toggle_like_view(request) -> web.Response:
    ...
    async with async_session() as session:
        ...
        if not debate.is_public:
            return web.json_response(
                {"error": "Cannot like private debates"}, status=400
            )
        ...
        if existing_like:
            ...
            await create_item(
                session,
                {"user_id": user_id, "debate_id": debate_id},
                db_models.UserLike,
            )
            is_liked = True
